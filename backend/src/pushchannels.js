...
const { isSafeHttpUrl, safeFetch } = require('./safefetch');
...
// ---------- Webhook SSRF 防护 ----------
// 具体实现在 safefetch.js（图片代理 / RSS 抓取共用同一套判定），本文件只做两件事：
//   保存设置时：isSafeHttpUrl 同步校验（协议白名单 + 内网/回环/云元数据字面量）
//   发送通知时：safeFetch 逐跳校验（重定向每一跳都重新校验地址 + DNS 解析结果）
// 只"发之前校验一次"是不够的：fetch 默认跟随 302，
// 对方回一个 Location: http://127.0.0.1:8080/... 就能把请求打进内网。
...
function sanitizeSettings(body) {
  ...
  if (typeof body.webhook === 'string') {
    ...
    if (!v) out.webhook = '';
    else if (/^https?:\/\/.+/.test(v) && isSafeHttpUrl(v)) out.webhook = v;
...
async function sendToUser(user, settings, payload) {
  ...
  if (c.webhook) {
    try {
      // safeFetch：3xx 一律不自动跟随，每一跳都重新校验（防"首跳公网 -> 302 到内网"）
